...
AVFormatContext *Init_ofmt_ctx(CameraParam *camerapar) {
  ...
  const char *in_filename = "../config/1.h264";
...
int read_h264_frame(char *data, int size, char **pp, int *pnb_start_code,
                    int fps, char **frame, int *frame_size, int *dts,
                    int *pts) {
  char *p = *pp;
...
void jrtplib_rtp_recv_thread(CameraParam *camerapar) {
  ...
  */
  ...
  std::map<uint16_t, Frame> Framemap;
    ...
    if (sess.GotoFirstSourceWithData()) {
      do {
        ...
        while ((pack = sess.GetNextPacket()) != NULL) {
          ...
          // printf("Got packet!\n");
          fwrite(pack->GetPayloadData(), pack->GetPayloadLength(), 1, fpH264);
          if (pack->GetPayloadType() == 96) {
            ...
            if (pack->HasMarker()) {
              ...
              int frame_len =
                  std::accumulate(
                      Framemap.begin(), Framemap.end(), 0,
                      [](int sum, auto a) { return sum + a.second.size; }) +
              ...
              */
              for (auto i = Framemap.begin(); i != Framemap.end(); i++) {
                memcpy(frame + size, i->second.data, i->second.size);
                size += i->second.size;
              }
#else
              int frame_len =
...
#endif
              // std::shared_ptr<char> ptr1(tmp, std::default_delete<char[]>());
              memcpy(frame + size, pack->GetPayloadData(),
                     pack->GetPayloadLength());
              /*
              for (auto i : Framemap)
                      LOG(INFO) << i.first << " "<< i.second.size;
              */
              int iPsLength;
              int pts_;
              int flag;

              GetH246FromPs(frame, frame_len, &returnps, &iPsLength, pts_,
                            flag);
              /*LOG(INFO) << "FRAMELEN: " << frame_len <<
                      "iPsLength: " << iPsLength;*/

              // printf("[%s]%x %x %x %x\n", __FUNCTION__, returnps[0],
              // returnps[1], returnps[2], returnps[3]);
              ...
              for (; p < returnps + iPsLength;) {
                ...
                if (nut == 7 || nut == 8 || nut == 1 || nut == 5) {
                  ...
                  all_len += size;
                  // LOG(INFO) << "type :" << (nut == 7 ? "SPS" : (nut == 8 ?
                  // "PPS" : (nut == 5 ? "I" : (nut == 1 ? "P" : (nut == 9 ?
                  // "AUD" : (nut == 6 ? "SEI" : "Unknown")))))) <<
                  //	" time :" << pts;
                  srs_human_trace(
                      "sent packet: type=%s, time=%d, size=%d, fps=%.2f, "
                      "b[%d]=%#x(%s)",
                      srs_human_flv_tag_type2string(SRS_RTMP_TYPE_VIDEO), pts,
                      size, 25, nb_start_code, (char)data[nb_start_code],
                      (nut == 7
                           ? "SPS"
                           : (nut == 8
                                  ? "PPS"
                  ...
                  LOG(INFO)
                      << "other, nut: " << int(nut) << "  " << data - returnps;
                }
                /*
                if (ret != 0) {
                        if (srs_h264_is_dvbsp_error(ret)) {
                                srs_human_trace("ignore drop video error,
                code=%d", ret);
                        }
                        else if (srs_h264_is_duplicated_sps_error(ret)) {
                                srs_human_trace("ignore duplicated sps,
                code=%d", ret);
                        }
                        else if (srs_h264_is_duplicated_pps_error(ret)) {
                                srs_human_trace("ignore duplicated pps,
                code=%d", ret);
                        }
                        else {
                                srs_human_trace("send h264 raw data failed.
                ret=%d", ret); LOG(ERROR) << "send h264 raw data failed. ret="
                << ret;
                        }
                }
                */
              ...
              */
...
#ifndef VEC
              for (auto pos = Framemap.begin(); pos != Framemap.end();) {
                if (pos->second.data != nullptr) {
                  delete[] pos->second.data;
                  pos->second.data = nullptr;
                }
                pos = Framemap.erase(pos);
              }
#else
              ...
              pack->GetPacketData()[15] == 0xc0)
              {
                                              ...
                                              else if (pack->GetPacketData()[12]
              == 0x00 && pack->GetPacketData()[13] == 0x00 &&
              pack->GetPacketData()[14] == 0x01 && pack->GetPacketData()[15] ==
              0xbd)
                                              {
              }
              else {
...
#ifndef VEC
              Framemap.insert(std::make_pair(
                  seq, Frame{static_cast<int>(pack->GetPayloadLength()), tmp}));
#else
          ...
          // fwrite(pack->GetPayloadData(), 1, pack->GetPayloadLength(),
          // fpH264);
